...
export default class AdditionalChargesFrom extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = { additional_charges_notes: props.additionalChargesNotes, additional_charges: props.additionalCharges || 0 }
  }
    ...
    if(this.state.showUpdate) {
      updateButton = (
        <Form.Item style={{ marginBottom: '0px', textAlign: 'center' }}>
          <Button type="primary" htmlType="submit">Actualizar cargos</Button>
        </Form.Item>
      )
    }
    ...
    return (
      <Form onSubmit={ this.processSubmit }>
        ...
        <Form.Item style={{ marginBottom: '0px' }}>
          <Input  type='textarea' value={ this.state.additional_charges_notes } name='additional_charges_notes' placeholder="Cargos adicionales" autosize onChange={ this.handleAdditionalChargesNotesChange }/>
        </Form.Item>

        <Form.Item style={{ marginBottom: '0px' }}>
          <InputNumber name='additional_charges' value={ this.state.additional_charges } max={9999999} min={0} formatter={ priceFormater } parser={ value => value.replace(/\$\s?|(,*)/g, '') } style={{ width: '100%' }} onChange={ this.handleAdditionalChargesChange }/>
        </Form.Item>
