    ...
    this.props.order.forEach( place_id => {
      ...
      if(abilitiesHelper.isMain() || (abilitiesHelper.isAdmin() && !abilitiesHelper.isAdmin(place))) {
        if(this.props.active) {
          activationButton = (
            <Popconfirm title="¿Seguro que quiere desactivar este placee?" onConfirm={ this.deactivate.bind(this, place) } okText="Sí" cancelText="No">
              <Button type="danger">
                Desactivar
              </Button>
            </Popconfirm>
          ...
          activationButton = (
            <Popconfirm title="¿Seguro que quiere reactivar este placee?" onConfirm={ this.activate.bind(this, place) } okText="Sí" cancelText="No">
              <Button type="primary">
                Activar
              </Button>
            </Popconfirm>
          ...
          <tr className="ant-table-row  ant-table-row-level-0" key={ 'cl-' + place.get('id') }>
            <td>
              <span className="ant-table-row-indent indent-level-0" style={{ paddingLeft: '0px'}}>
                <Link to={ '/clients/' + place.get('client_id') }>
                  { place.get('name') }
                </Link>
                <div>
